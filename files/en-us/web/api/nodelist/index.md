...
**`NodeList`** objects are collections of [nodes](/en-US/docs/Web/API/Node), usually returned by properties such as {{domxref("Node.childNodes")}} and methods such as {{domxref("document.querySelectorAll()")}}. only {{domxref("document.querySelectorAll()")}} returns a static NodeLis, others APIs returns a live NodeLis.
